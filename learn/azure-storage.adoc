...
[.lead]
Astra Control Service supports Azure NetApp Files or Azure Disk Storage as the backend storage for Azure Kubernetes Service (AKS) clusters.
...
Astra Control Service supports Azure managed disks as another backend storage option. This service provides block-level storage that is managed by Azure.
