...
| CVS | Apps with low-to-medium storage performance needs | https://cloud.netapp.com/cloud-volumes-global-regions#cvsGcp[View supported Google Cloud regions^] | netapp-cvs-standard | 100 GiB
...
Astra provisions persistent volumes using a minimum volume size of 100 GiB with the CVS service type, even if the PVC asks for a smaller volume size.
