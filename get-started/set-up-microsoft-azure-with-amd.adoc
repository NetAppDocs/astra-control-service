...
Ensure that clusters are healthy and running Kubernetes version 1.21.x to 1.23, that node pools are online and running *Linux*, and more. <<Azure Kubernetes Service cluster requirements,Learn more about this step>>.
...
[role="quick-margin-para"]
By default, Azure containers use the Locally Redundant Storage (LRS) redundancy option. As an optional step, you can configure a more durable level of redundancy for Azure buckets. <<Optional: Configure redundancy for Azure backup buckets,Learn more about this step>>.
...
Kubernetes version:: Clusters must be running Kubernetes version 1.21.x to 1.23.
...
You can configure a more durable redundancy level for Azure backup buckets. By default, Azure containers use the Locally Redundant Storage (LRS) redundancy option. To use a more durable redundancy option for Azure buckets, you need to do the following:
...
. Add the container as a bucket to Astra Control Service. See https://docs.netapp.com/us-en/astra-control-service/use/manage-buckets.html#add-an-additional-bucket[Add an additional bucket].
. (Optional) To use the newly created bucket as the default bucket for Azure backups, set it as the default bucket for Azure. See https://docs.netapp.com/us-en/astra-control-service/use/manage-buckets.html#change-the-default-bucket[Change the default bucket].
