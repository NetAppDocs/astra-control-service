...
* Kubernetes Engine Admin
+
To list clusters and create admin access to manage apps.

* Cloud Volumes Admin
+
To manage persistent storage for apps.

* Cloud Storage Admin
+
To manage buckets and objects for backups of apps.

* Service Usage Viewer
+
To check if the required Cloud Volumes Service for Google Cloud APIs are enabled.

* Compute Network Viewer
+
To check if the Kubernetes VPC is allowed to reach Cloud Volumes Service for Google Cloud.
...
* Service Management API
+
The last two APIs are required specifically for Cloud Volumes Service for Google Cloud.
