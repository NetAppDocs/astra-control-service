...
Astra uses Cloud Volumes Service for Google Cloud as the backend storage for your persistent volumes. Other than the APIs that you enabled in the previous step, the only other requirement is to set up networking peering from your VPC to Cloud Volumes Service.
...
. Log in to Google Cloud Platform and go to *Cloud Volumes*.

. In the *Volumes* page, click *Create*.
+
You'll only need to enter your networking information to obtain the commands.
...
. Click the buttons to view the commands.
