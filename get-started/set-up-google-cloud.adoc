...
<<Create a service account key,Follow step-by-step instructions>>.
...
[role="quick-margin-para"]
Set up private services access for Cloud Volumes Service for Google Cloud.

<<Enable private services access,Follow step-by-step instructions>>.
...
If you'd rather use the gcloud CLI, here are the commands:
...
Project Astra uses Cloud Volumes Service for Google Cloud as the backend storage for your persistent volumes. Other than the APIs that you enabled in the previous step, the only other requirement is to enable private services access to Cloud Volumes Service.
