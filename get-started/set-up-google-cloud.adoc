...
Storage pools:: If you use NetApp Cloud Volumes Service as a storage backend and plan on using the CVS service type, you need to configure storage pools before you can provision volumes. Refer to link:../learn/choose-class-and-size.html#overview[Service type, storage classes, and PV size for GKE clusters^] for more information.
