...
Ensure that clusters are healthy and running a supported Kubernetes version, that worker nodes are online and running Container-Optimized OS or Ubuntu, and more. <<GKE cluster requirements,Learn more about this step>>.
...
Kubernetes version:: A cluster must be running a Kubernetes version in the range of 1.22 to 1.24.
