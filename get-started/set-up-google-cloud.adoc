...
If using the CVS service type, configure storage pools:: If you plan on using the `CVS` service type for your workloads, you need to configure storage pools before you can provision volumes. Refer to link:../learn/choose-class-and-size.html[Service type, storage classes, and PV size for GKE clusters^] for more information.
