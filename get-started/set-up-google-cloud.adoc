...
Ensure that clusters are healthy and running a Kubernetes version in the range of 1.19 to 1.21, that worker nodes are online and running Container-Optimized OS or Ubuntu, and more. <<GKE cluster requirements,Learn more about this step>>.
...
* NetApp Cloud Volumes Service (Recommended, but only required if you plan to use Cloud Volumes Service for Google Cloud)
...
* NetApp Cloud Volumes Admin (Recommended, but only required if you plan to use Cloud Volumes Service for Google Cloud)
* Storage Admin
...
Kubernetes version:: A cluster must be running a Kubernetes version in the range of 1.19 to 1.21.
