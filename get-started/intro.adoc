...
+
** If you use a NetApp cloud service storage offering as your storage backend, Astra Control Service uses Astra Trident to provision persistent volumes for your apps. If you use the Azure or AWS storage service as your storage backend, you might need to install a provider-specific CSI driver. Installation instructions are outlined in the link:set-up-amazon-web-services.html[AWS^], link:set-up-microsoft-azure-with-amd.html[Azure with Azure Managed Disks^], and link:set-up-microsoft-azure-with-anf.html[Azure with Azure NetApp Files^]set up steps.
