...
[role="quick-margin-para"]
Astra Control Service does not install Astra Trident automatically for EKS clusters. Configure Astra Trident if you plan to use the FSxN storage backend service.
...
CSI drivers:: If you use the Amazon Elastic Block Store (EBS) storage backend, you need to install the CSI driver for EBS. For instructions, see https://docs.aws.amazon.com/eks/latest/userguide/ebs-csi.html[Amazon EBS CSI driver^].
...
== Optional: Configure Astra Trident for Amazon FSx for NetApp ONTAP
If you use the Amazon FSx for NetApp ONTAP (FSxN) storage backend, you need to install Astra Trident. For instructions, see https://docs.netapp.com/us-en/trident/trident-get-started/kubernetes-deploy.html[Astra Trident deployment overview^].

For details about using Astra Trident with FSxN, see https://docs.netapp.com/us-en/trident/trident-use/trident-fsx.html[Use Astra Trident with Amazon FSx for NetApp ONTAP^].
