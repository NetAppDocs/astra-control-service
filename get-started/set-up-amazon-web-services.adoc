...
[role="quick-margin-para"]
Create an Amazon Identity and Access Management (IAM) user and associate it with a Kubernetes service account.
...
[role="quick-margin-para"]
Save the credentials for the IAM user so that you can make Astra Control Service aware of the new user.
...
.Result
The `credential.json` file is created, and you can use it to create a user in Astra Control Service that can interact with your Amazon Web Services EKS clusters.
