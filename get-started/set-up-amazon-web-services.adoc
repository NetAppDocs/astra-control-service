...
CSI drivers for Amazon Elastic Block Store (EBS):: If you use the Amazon EBS storage backend, you need to install the CSI driver for EBS (it is not installed automatically).
...
. The Amazon EBS CSI driver isn't installed when you first create a cluster. To use the driver, you must add it as an Amazon EKS add-on or as a self-managed add-on. Do one of the following:
+
* Add the driver as an Amazon EKS add-on. See https://docs.aws.amazon.com/eks/latest/userguide/managing-ebs-csi.html[Managing the Amazon EBS CSI driver as an Amazon EKS add-on^].
* Add the driver as a self-managed add-on. See https://github.com/kubernetes-sigs/aws-ebs-csi-driver[Amazon EBS Container Storage Interface (CSI) driver^].
...
. Clone the Amazon EBS Container Storage Interface (CSI) driver GitHub repository to your system.
+
----
...
Configure Astra Trident for Amazon FSx for NetApp ONTAP::
If you use the Amazon FSx for NetApp ONTAP storage backend, you need to install Astra Trident. For instructions, see https://docs.netapp.com/us-en/trident/trident-get-started/kubernetes-deploy.html[Astra Trident deployment overview^].

For details about using Astra Trident with FSx for NetApp ONTAP, see https://docs.netapp.com/us-en/trident/trident-use/trident-fsx.html[Use Astra Trident with Amazon FSx for NetApp ONTAP^].
