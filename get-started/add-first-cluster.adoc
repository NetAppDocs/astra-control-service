...
ifndef::azure,gcp[]
If you don't have a cluster yet, you can <<Add a cluster by uploading a kubeconfig file>>, or create one that meets link:set-up-amazon-web-services.html#eks-cluster-requirements[Astra Control Service requirements for Amazon Elastic Kubernetes Service (EKS)].
endif::azure,gcp[]
ifndef::azure,aws[]
If you don't have a cluster yet, you can <<Add a cluster by uploading a kubeconfig file>>, or create one that meets link:set-up-google-cloud.html#gke-cluster-requirements[Astra Control Service requirements for Google Kubernetes Engine (GKE)].
endif::azure,aws[]
ifndef::gcp,aws[]
If you don't have a cluster yet, you can <<Add a cluster by uploading a kubeconfig file>>, or create one that meets link:set-up-microsoft-azure-with-anf.html#azure-kubernetes-service-cluster-requirements[Astra Control Service requirements for Azure Kubernetes Service (AKS) with Azure NetApp Files] or link:set-up-microsoft-azure-with-amd.html#azure-kubernetes-service-cluster-requirements[Astra Control Service requirements for Azure Kubernetes Service (AKS) with Azure managed disks].
...
ifdef::gcp+azure+aws[]
If you don't have a cluster yet, you can <<Add a cluster by uploading a kubeconfig file>>, or create one that meets the requirements of one of the following providers:
...
== Add a cluster by uploading a kubeconfig file
You can add an on-premises or cloud-hosted Kubernetes cluster by uploading the cluster kubeconfig file to Astra Control Service. You'll need to make sure the cluster meets certain requirements.

=== What you'll need before you upload the kubeconfig file
[%collapsible]
====
Make sure that your cluster meets the following requirements and that you perform eligibility checks before you add it to Astra Control Service by uploading a kubeconfig file.
...
.. *Other*: Provide details about your private cluster credentials by uploading a JSON file or by pasting the contents of that JSON file from your clipboard.

. *Cloud name* (For Azure, Google Cloud Platform, and Amazon Web Services clusters): Provide a name for the new cloud instance that will be created when you add this cluster. Learn more about link:../use/manage-cloud-instances.html[cloud instances].

. *Credential name* (For clusters of type "Other"): Provide a name for the private cluster credential you are uploading to Astra Control.
