...
ifdef::aws[]
* For Amazon Web Services, you should have the service account key file for a service account that has the required permissions. link:../get-started/set-up-amazon-web-services.html#create-a-service-account[Learn how to set up a service account].
endif::aws[]
ifdef::gcp[]
...
ifdef::aws[]
.. *Amazon Web Services*: Provide details about your Amazon Web Services account by uploading a JSON file or by pasting the contents of that JSON file from your clipboard.
+
The JSON file should contain the output from the AWS CLI when you created the account. It can also include your subscription ID so it's automatically added to Astra Control. Otherwise, you need to manually enter the ID after providing the JSON.
endif::aws[]
ifdef::azure[]
...
* Amazon FSx: No price, performance, or resilience information available
====
+
...
* https://docs.aws.amazon.com/fsx/index.html[Amazon FSx^]
endif::aws[]
+
ifdef::aws[]
