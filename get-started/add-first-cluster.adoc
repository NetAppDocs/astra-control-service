...
.. *Other*: Provide details about your cluster by uploading a `kubeconfig.yaml` file or by pasting the contents of the `kubeconfig.yaml` file from your clipboard. For instructions and requirements, see <<Add a cluster by uploading a kubeconfig file>>. 
...
. *Credential name* (For clusters of type "Other"): Provide a name for the cluster credential you are uploading to Astra Control. For instructions and requirements, see <<Add a cluster by uploading a kubeconfig file>>.
...
== Add a cluster by uploading a kubeconfig file
You can add an on-premises or cloud-hosted Kubernetes cluster by uploading the cluster kubeconfig file to Astra Control Service. You'll need to make sure the cluster meets certain requirements and perform certain tasks to prepare the cluster before you add it.
