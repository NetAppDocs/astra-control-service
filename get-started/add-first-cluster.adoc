...
[%collapsible]
=======
Your self-managed cluster can use CSI drivers or it can use NetApp storage with Astra Trident. The below instructions give details surrounding an Astra Trident configuration for storage. For more information on using CSI drivers to integrate storage, refer to the documentation for your storage provider.

NOTE: If there is no existing Astra Trident installation on your self-managed cluster that is hosted in either AKS or GCP, Astra Control Service automatically installs Astra Trident when you add the cluster.
