...
+
NOTE: If you select a cluster that has not been configured for a cloud provider, a warning appears. This cluster does not have NetApp Trident installed and configured. You can proceed with adding the cluster if you intend to use a different storage service.
...
*For clusters added using a `kubeconfig.yaml` file*:
Astra Control Service creates an admin account on the cluster and sets the default storage class that you specified. This process can take up to 5 minutes.
