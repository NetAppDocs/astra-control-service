...
* Use the Astra Control web UI to assign a different default storage class. refer to <<Change the default storage class>>.
======================
...
NOTE: If there is no existing Astra Trident installation on your self-managed cluster that is hosted in either Azure Kubernetes Service or Google Cloud, Astra Control Service automatically installs Astra Trident when you add the cluster.

Your self-managed cluster needs to meet the following requirements:

* The cluster must be accessible via the internet
*	A version of Astra Trident that is link:../get-started/requirements.html#operational-environment-requirements[supported by Astra Control Service^] is installed:
+
