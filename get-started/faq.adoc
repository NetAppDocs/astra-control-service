...
Astra aims to simplify your application data lifecycle management operations for Kubernetes native applications. Astra supports Kubernetes clusters running on Google Kubernetes Engine (GKE) and Azure Kubernetes Service (AKS).
...
*Can I add a private GKE cluster to Astra?*
...
Yes, Astra can manage clusters that reside in a shared VPC.
...
*What happens to my applications and data after removing the cluster from Astra?*

Removing a cluster from Astra will not make any changes to the cluster's configuration (applications and persistent storage). Any Astra snapshots or backups taken of applications on that cluster will be unavailable to restore. Volume snapshot data stored within the backend storage will not be removed. Persistent Storage backups created by Astra will remain within your cloud provider's object store, but they are unavailable for restore.
...
*Will NetApp Trident be uninstalled when I remove a cluster from Astra?*
...
The NetApp Trident operator sets the default storage class to `netapp-cvs-premium` after it's successfully added to Astra. When an application's PVCs are not bound to Cloud Volumes Service for Google Cloud, there are a few steps that you can take:
...
*Can I manage snapshots taken by Astra directly through my cloud provider?*
