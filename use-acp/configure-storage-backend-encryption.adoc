...
. On the managed cluster, create a storage backend configuration file using the following examples, depending on where you need to define the storage backend (storage backend level, or virtual pool level). Note that you can specify one of three values for the `spec.kerberos` (storage backend level) or `spec.storage.kerberos` (virtual pool level) field:
