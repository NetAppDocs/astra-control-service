...
.About this task
You can create a configuration file that should be applied at the storage backend level using the `spec.kerberos` field, or at the virtual pool level using the `spec.storage.kerberos` field. When you define the configuration at the virtual pool level, the pool is selected using the label in the storage class.

Note that you can specify one of three different versions of encryption at either level:
...
. On the managed cluster, create a storage backend configuration file using one of the following examples, depending on where you need to define the storage backend (storage backend level or virtual pool level). 
...
spec:
  ...
  subnet: <SUBNET>
  nasType: nfs
  kerberos: sec=krb5i #can be krb, krb5i, or krb5p
  credentials:
    name: backend-tbc-anf-secret
----
--
