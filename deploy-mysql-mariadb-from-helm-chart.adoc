...
This guide focuses on Helm as the preferred way to deploy MySQL/MariaDB apps. Plain YAML and Operator-based deployments may be covered in future guides.
...
For the Project Astra alpha program, we recommend the https://hub.helm.sh/charts/bitnami/mariadb[Bitnami MariaDB chart] and https://hub.helm.sh/charts/bitnami/mysql[Bitnami MySQL chart]. To install these charts, follow the instructions on the Bitnami Helm chart pages for `helm repo add` and `helm install`.
...
Before you run the Helm charts for real, you need to create the namespace. Here is the kubectl command to create the `my-test-namespace` used in the previous example:
