...
* <<In-place restore operations to nas-economy storage backends fail>>
...
If you perform an in-place restore of an application (restore the app to the original namespace it was backed up from), and the app's storage class uses the `ontap-nas-economy` driver, the restore operation fails. As a workaround, restore to a different namespace.
