...
* <<Restore operations when using Kerberos in-flight encryption can fail>>
...
When you restore an application from backup to a storage backend that is using Kerberos in-flight encryption, the restore operation can fail. As a workaround, restore the application from a snapshot or replicate the application data using NetApp SnapMirror.
