...
Astra Control Service now supports Google Persistent Disk and Azure managed disks as backend storage options.

link:../get-started/set-up-google-cloud.html[Set up Google Cloud].
link:../get-started/set-up-microsoft-azure.html[Set up Microsoft Azure].
...
Astra Control supports custom scripts that you can run before or after a snapshot is taken of an app. This enables you to perform tasks like suspending database transactions so that the snapshot of your database app is consistent.

link:../use/manage-app-execution-hooks.html[Manage app execution hooks].

=== Operator-deployed apps
Astra Control supports several apps when they are deployed with operators.
...
link:../get-started/set-up-microsoft-azure.html#create-an-azure-service-principal-2[Create an Azure service principal].
