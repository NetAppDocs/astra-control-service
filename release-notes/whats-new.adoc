...
Astra now supports shared VPCs in Google Cloud. This support enables you to manage GKE clusters that reside in a shared VPC.
...
link:choose-class-and-size.html[Learn how Astra uses Cloud Volumes Service for Google Cloud as the backend storage for persistent volumes].

=== Support for Container-Optimized OS

The Container-Optimized OS is now supported with GKE worker nodes. This is in addition to support for Ubuntu.

link:set-up-google-cloud.html[Learn more about GKE cluster requirements].
