...
*** *Backup and restore capabilities for applications with `ontap-nas-economy` driver-backed storage backends*: link:../use/protect-apps.html#enable-backup-and-restore-for-ontap-nas-economy-operations[Enable backup and restore operations for `ontap-nas-economy`^] with some simple steps.
...
* Astra Control now supports link:../get-started/add-first-cluster.html[on-premise Red Hat OpenShift Container Platform clusters^].
