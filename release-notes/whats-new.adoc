...
* Astra Control Service now supports link:../get-started/add-first-cluster.html[on-premise Red Hat OpenShift Container Platform clusters^] as well as private link:../get-started/install-astra-connector.html[Red Hat OpenShift Service on AWS (ROSA) clusters^].
