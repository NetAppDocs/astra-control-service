...
Learn how to automate application data lifecycle management using the REST API (*_LINK COMING SOON_*).
...
=== Min volume size of 100 GiB

Astra now creates Persistent Volumes with a minimum size of 100 GiB when using the CVS service type with GKE clusters. With this change, 100 GiB is now the default volume size for all types of backend storage (Google Cloud and Microsoft Azure).

* link:../learn/choose-class-and-size.html[Learn more about storage for GKE clusters]
* link:../learn/azure-storage.html[Learn more about storage for AKS clusters]
