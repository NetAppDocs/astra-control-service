...
=== Invite user improvements
You can now invite up to 10 users at a time.

=== Namespace role-based access control (RBAC)
Astra Control Service now supports assigning namespace constraints to Member or Viewer users. For more information, see link:../learn/user-roles-namespaces.html[User roles and namespaces].

=== Support for private AKS clusters
You can now manage AKS clusters that use private IP addresses. For more information, see link:../get-started/add-first-cluster.html[Start managing Kubernetes clusters from Astra Control Service].
