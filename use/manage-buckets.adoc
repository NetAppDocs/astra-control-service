...
Astra Control Service automatically populates the cloud instance's default bucket as the bucket that will be used for ad-hoc backups and for ad-hoc clones when you don't choose to clone from an existing backup. You can choose a different bucket before starting the backup or clone operation.
