...
* The execution hooks feature is disabled by default for new Astra Control deployments. Owner or Admin users can enable or disable the execution hooks feature for all accounts. The feature must be enabled before it can be used. The enablement status is preserved during Astra Control upgrades. Refer to <<Enable the execution hooks feature>> and <<Disable the execution hooks feature>> for instructions.
...
If you are an Owner or Admin user, you can enable the execution hooks feature. When you enable the feature, all Owner and Admin users can use execution hooks and all users can view existing execution hooks and hook scripts.
...
If you are an Owner or Admin user, you can disable the execution hooks feature. You must delete all existing execution hooks before you can disable the execution hooks feature. Refer to <<Delete an execution hook>> for instructions on deleting an existing execution hook.
