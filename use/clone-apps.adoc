...
** During an app backup or app restore, you can optionally specify a bucket ID. A bucket ID is required when you clone across clusters, but is optional when cloning within the same cluster. 
** The cloud instance containing the source cluster of the clone operation must have a default bucket set. 
