...
After Astra Control discovers namespaces on your clusters, you can define applications that you want to manage. You can choose to link:../learn/app-management.html[manage an entire namespace as a single application or manage resources of an app in one or more namespaces]. It all comes down to the level of granularity that you need for data protection operations.
...
. To include cluster-scoped resources in addition to those that Astra Control automatically includes, check *Include additional cluster-scoped resources* and complete the following: 
