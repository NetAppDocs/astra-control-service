...
. Log into the Astra Control Center registry:
+
NOTE: Docker commands are used in these examples. Podman commands can also be used.
+
----
docker login <FQDN of Astra Control Service> -u <API token>
----

. Pull the Astra Control Provisioner image from the registry:
+
----
docker pull <FQDN of Astra Control Service>/acp-image:latest
----
. Run the image. This extracts the contents into your current working directory:
+
----
docker run --rm -v "$(pwd):/package" <FQDN of Astra Control Service>/acp-image:latest
----
