...
. Customize the name of the backup.
. Choose whether to back up the app from an existing snapshot. If you select this option, you can choose from a list of existing snapshots.
. Choose a destination bucket for the backup from the list of storage buckets. A write once read many (WORM) bucket is shown with a status of "Locked" when you hover over or select the bucket.
+
...
NOTE: If a backup is immutable, its status is "Locked" in the Astra Control web UI.
